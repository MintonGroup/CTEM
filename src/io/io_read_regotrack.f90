...
subroutine io_read_regotrack(user,surf)
   ...
   ! Open files with mixing information
   !open(LUP,file='mixtop.dat',status='old',form='unformatted',recl=recsize,access='direct')
   !read(LUP,rec=1) mix_num
   !close(LUP)
   !recsize = sizeof(dtmp) * user%gridsize * user%gridsize
   !open(LUP,file='mixdmap.dat',status='old',form='unformatted',recl=recsize,access='direct')
   !read(LUP,rec=1) mixdep
   !close(LUP)   
   !open(LUP,file='excavdmap.dat',status='old',form='unformatted',recl=recsize,access='direct')
   !read(LUP,rec=1) excavdep
   !close(LUP)
   ...
   do j=1,user%gridsize
      do i=1,user%gridsize
         ...
         deallocate(regotopi,melti,compi)

         !surf(i,j)%nmix = mix_num(i,j)
         !surf(i,j)%dmix = mixdep(i,j)
         !surf(i,j)%dexcav = excavdep(i,j)
