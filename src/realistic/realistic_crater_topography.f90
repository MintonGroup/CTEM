subroutine realistic_make_a_crater(user,surf,crater,deltaMtot)
   !----------------------------------------------------------------------------------------------------------
   ! This is the Fortran code to generate a realistic crater. At this moment, it can generate the outlines of
   ! rim, floor, and ejecta, based on their one-dimensional power spectral density.
   !----------------------------------------------------------------------------------------------------------
   use module_globals
   use module_util
   use module_crater
   use module_realistic, EXCEPT_THIS_ONE => realistic_make_a_crater
   implicit none
   ! in and out
   ...
   ! internal
   type(psdtype) :: psd_distance,psd_elevation                                                  
   real(DP) :: newdem,elchange,rad_roi,xbar,ybar
   integer(I4B) :: xpi,ypi,i,j,inc 
   real(DP) :: arc_length,radial_distance,random
   real(DP) :: rim_distance,rim_distance_delta,rim_elevation,rim_elevation_delta             
   ...
   call realistic_calculate_am_wl_phase_from_diameter(psd_distance,amplitude_rim_distance,wavelength_rim_distance,phase_rim_distance)
   ...
   psd_distance%misfit_k_b_sigma=0.000000_DP
   call realistic_calculate_am_wl_phase_from_diameter(psd_distance,amplitude_floor_distance,wavelength_floor_distance,phase_floor_distance)
   ...
   psd_distance%misfit_k_b_sigma=0.000000_DP
   call realistic_calculate_am_wl_phase_from_diameter(psd_distance,amplitude_ejecta_distance,wavelength_ejecta_distance,phase_ejecta_distance)
   ! PSD of elevation
   ...
   call realistic_calculate_am_wl_phase_from_diameter(psd_elevation,amplitude_rim_elevation,wavelength_rim_elevation,phase_rim_elevation)
   ...
   do j = -inc,inc
      do i = -inc,inc
         ...
         call realistic_make_an_outline(arc_length,psd_distance ,amplitude_rim_distance   ,wavelength_rim_distance   ,phase_rim_distance   ,rim_distance_delta)
         call realistic_make_an_outline(arc_length,psd_distance ,amplitude_floor_distance ,wavelength_floor_distance ,phase_floor_distance ,floor_distance_delta)
         call realistic_make_an_outline(arc_length,psd_distance ,amplitude_ejecta_distance,wavelength_ejecta_distance,phase_ejecta_distance,ejecta_distance_delta)
         call realistic_make_an_outline(arc_length,psd_elevation,amplitude_rim_elevation  ,wavelength_rim_elevation  ,phase_rim_elevation  ,rim_elevation_delta)
         ...
         ! If some weird values are obtianed for floor_distance, ejecta_distance, rim_elevation
         if (floor_distance>rim_distance) then
            print *,'Warning: A weird floor_distance!',j,i,floor_distance,rim_distance
            ! floor_distance=rim_distance
         endif
         if (ejecta_distance<rim_distance) then
            print *,'Warning: A weird ejecta_distance!',j,i,ejecta_distance,rim_distance
            ! floor_distance=rim_distance
         endif
         if (rim_elevation<0.0_DP) then
            print *,'Warning: A negative rim elevation!',j,i,rim_elevation 
         endif
...
subroutine realistic_calculate_am_wl_phase_from_diameter(psd_1D,amplitude,wavelength,phase)
   use module_globals
   use module_realistic, EXCEPT_THIS_ONE => realistic_calculate_am_wl_phase_from_diameter
   implicit none
   ! in and out
   ...
   integer(I4B),parameter :: outfile = 15
   ! excutable
   call realistic_calculate_breakpoint_slope_from_diameter(psd_1D)
   call realistic_calculate_targetPSD_from_breakpoint_slope(psd_1D,wavelength,psd)
   call realistic_calculate_am_wl_phase_from_targetPSD(psd_1D,wavelength,psd,amplitude,phase)
...
subroutine realistic_calculate_breakpoint_slope_from_diameter(psd_1D)
   use module_globals
   use module_realistic, EXCEPT_THIS_ONE => realistic_calculate_breakpoint_slope_from_diameter
   use module_util
   implicit none
   ! in and out
   type(psdtype),intent(inout) :: psd_1D
...
subroutine realistic_calculate_targetPSD_from_breakpoint_slope(psd_1D,wavelength,psd)
   use module_globals
   use module_realistic, EXCEPT_THIS_ONE => realistic_calculate_targetPSD_from_breakpoint_slope
   use module_util
   implicit none
   !in and out
...
subroutine realistic_calculate_am_wl_phase_from_targetPSD(psd_1D,wavelength,psd,amplitude,phase)
   use module_globals
   use module_realistic, EXCEPT_THIS_ONE => realistic_calculate_am_wl_phase_from_targetPSD
   implicit none
   !in and out
   type(psdtype), intent(in)  :: psd_1D
...
!    integer, dimension(:), allocatable :: seed
  ...
  !----------------------------------------------------------------------------------------------------------------------
   do i = 1, psd_1D%num_sine
    ...
    !   print *, i,phase_random,phase(i) 
   end do
end subroutine realistic_calculate_am_wl_phase_from_targetPSD


subroutine realistic_make_an_outline(arc_length,psd_1D,amplitude,wavelength,phase,noise_component)
   use module_globals
   use module_realistic, EXCEPT_THIS_ONE => realistic_make_an_outline
   implicit none
   ! in and out
